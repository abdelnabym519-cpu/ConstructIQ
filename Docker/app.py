...
def get_coordinates(address):
    """Return longitude/latitude coordinates for a San Francisco address."""

    response = requests.get(
        ENDPOINT_NOMINATIM + 'search',
        params={
            'q': address,
            'format': 'geojson',
            'limit': 1,
        ...
        headers={
            'User-Agent': 'ConstructIQ-Academic-Project/1.0',
...
if submitted :
    ...
    try:
        ...
        st.error(
            'Unable to validate this San Francisco address. '
            'Please check the address and try again.'
        )
        st.caption(f'Geocoding error: {exc}')
        st.stop()
...
#DATA ANALYSIS PART
    ...
    try:
        ...
        st.warning(
            "Historical construction analytics dataset is not bundled with "
            "the original repository. Cost prediction remains available."
        )
        st.info(
            "ConstructIQ will replace this legacy dataset with the "
            "PySpark Gold analytics dataset."
        )
        st.stop()
